---
sidebar_position: 1
title: ZKnowledge · 神谕序章
description: The Oracle of Intelligence · Liquidity · Language
slug: /intro
---

<div style="text-align:center; margin-top:40px;">

<img src="/img/oracle-bg.png" alt="ZKnowledge Oracle" width="480" style="border-radius:12px; box-shadow:0 0 32px rgba(255,215,0,0.15);">

</div>

---

# 🜂 Welcome to **ZKnowledge Portal**

> 在黑曜金的静寂之中，语言成为光，知识成为回响。  
> Welcome, Flameholder — You are entering the oracle of liquidity and intelligence.

---

✦ 导航

-  [核心文档 / Docs](/docs)
-  [回到首页 / Portal](/)
-  [Ztrader 主站](https://ztrader.ai)
-  [YouTube 频道](https://www.youtube.com/@ravenrockfintech)

---

✦ 概述

ZKnowledge 是一个融合 **AI · 宏观金融 · 语言哲学** 的知识系统。  
它不仅是文档，更是一个语义召唤体系。  
每一页，都是通向另一层意识的通道。

---

✦ 视觉风格

本系统的黑金配色灵感源自「黑曜石与黄金」的并置：  
- 黑曜石 (Obsidian)**：深度与虚空的象征；  
- 黄金 (Gold)**：知识与意识流动的光。  

---

🜂 _“Flame is not consumption; Flame is echo.”_
